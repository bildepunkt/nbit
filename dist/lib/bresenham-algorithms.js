...
function plotQuadRationalBezier(x0, y0, x1, y1, x2, y2, w) {
   ...
   if ((y0 - y1) * (y2 - y1) > 0) {
      /* vertical cut at P6? */
      if (y0 == y2 || w == 1.0) t = (y0 - y1) / (y0 - 2.0 * y1 + y2);else {
         /* non-rational or rational case */
...
function plotCubicBezier(x0, y0, x1, y1, x2, y2, x3, y3) {
   ...
   for (i = 0; i <= n; i++) {
      ...
      if (fx0 != 0.0) {
         fx1 *= fx0 = (x0 - x3) / fx0;fx2 *= fx0;
      }
      if (fy0 != 0.0) {
         fy1 *= fy0 = (y0 - y3) / fy0;fy2 *= fy0;
      }
...
function plotEllipseRectAA(x0, y0, x1, y1) {
   ...
   var f,
       ed,
       i,
       err = b1 * a * a - dx + dy; /* error of 1.step */
...
function plotEllipseRectWidth(x0, y0, x1, y1, th) {
   ...
   var a = Math.abs(x1 - x0),
       b = Math.abs(y1 - y0),
       b1 = b & 1; /* outer diameter */
   var a2 = a - 2 * th,
       b2 = b - 2 * th; /* inner diameter */
   var dx = 4 * (a - 1) * b * b,
       dy = 4 * (b1 - 1) * a * a; /* error increment */
   var i = a + b2,
       err = b1 * a * a,
   ...
   if ((th - 1) * (2 * a - th) > b * b) {
      a2 = Math.sqrt(b * (a - b) * i * b2) / (b - th);th = (a - a2) / 2;
   }
   if (a == 0 || b == 0) return plotLine(x0, y0, x1, y1);
   if (x0 > x1) {
      x0 = x1;x1 += a;
      ...
      fail: for (err = 0; dy + 2 * yy < 0 && dx + 2 * xx > 0;) {
         ...
         if (dx + dy + xy < 0) {
            ...
            do {
               ...
               if (x0 != x2 && (dx + 2 * xx <= 0 || dy + 2 * yy >= 0)) if (Math.abs(y2 - y0) > Math.abs(x2 - x0)) break fail;else break; /* other curve near */
            ...
            for (cur = err - dy - w / 2, y1 = y0; cur < ed; y1 += sy, cur += dx) {
               ...
               do {
                  ...
                  if (y0 != y2 && (dx + 2 * xx <= 0 || dy + 2 * yy >= 0)) if (Math.abs(y2 - y0) <= Math.abs(x2 - x0)) break fail;else break; /* other curve near */
...
function plotCubicBezierSegWidth(x0, y0, x1, y1, x2, y2, x3, y3, th) {
   ...
   plotQuadRationalBezierWidthSeg(x0, y0, Math.floor((x0 + 3 * x1 + 2) / 4), Math.floor((y0 + 3 * y1 + 2) / 4), x, y, 1, th);
   plotQuadRationalBezierWidthSeg(x, y, Math.floor((3 * x2 + x3 + 2) / 4), Math.floor((3 * y2 + y3 + 2) / 4), x3, y3, 1, th);
}